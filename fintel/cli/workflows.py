...
custom workflows that can be used with ``fintel analyze`` and
``fintel batch`` via the ``custom:<workflow_id>`` analysis type.
"""
...
def workflows(verbose: bool):
    ...
    \b
      fintel analyze AAPL --analysis-type custom:<workflow_id>
      fintel batch tickers.csv --analysis-type custom:<workflow_id>
    ...
    console.print()
    console.print(
        "[dim]Usage:[/dim]  fintel analyze AAPL --analysis-type [cyan]custom:<workflow_id>[/cyan]"
    )
    console.print(
        "[dim]       [/dim] fintel batch tickers.csv --analysis-type [cyan]custom:<workflow_id>[/cyan]"
    )
