...
# -*- coding: utf-8 -*-
"""
Configuration management for EON using Pydantic Settings.
"""
...
class EonConfig(BaseSettings):
    """
    Main configuration class for EON.

    Loads settings from environment variables and .env file.
    All settings can be overridden with EON_ prefix.

    Example:
        export EON_DATA_DIR=/path/to/data
        export EON_NUM_WORKERS=10
    """
    ...
    def _validate_config(self):
        ...
        if self.sec_user_email == "user@example.com":
            errors.append(
                "Please set a valid EON_SEC_USER_EMAIL in your .env file. "
        ...
        if errors:
            from eon.core.exceptions import ConfigurationError
...
_config_instance: Optional[EonConfig] = None
_config_lock = threading.Lock()


def get_config(**kwargs) -> EonConfig:
    """
    ...
    if _config_instance is None:
        with _config_lock:
            ...
            if _config_instance is None:
                _config_instance = EonConfig(**kwargs)
