...
DEFAULT_LOG_MAX_SIZE_MB = int(os.environ.get("EON_LOG_MAX_SIZE_MB", "10"))
DEFAULT_LOG_BACKUP_COUNT = int(os.environ.get("EON_LOG_BACKUP_COUNT", "5"))
DEFAULT_LOG_FILE = os.environ.get("EON_LOG_FILE", None)
...
def setup_logging(
    ...
    Args:
        ...
        log_file: Path to log file (if None, uses default or EON_LOG_FILE env)
    ...
    Environment variables:
        EON_LOG_FILE: Default log file path
        EON_LOG_MAX_SIZE_MB: Maximum log file size in MB (default: 10)
        EON_LOG_BACKUP_COUNT: Number of backup files to keep (default: 5)
    """
    ...
    if log_to_file:
        ...
        if log_file is None:
            if DEFAULT_LOG_FILE:
                ...
                if process_id is not None:
                    log_file = Path(f"logs/eon_process_{process_id}_{timestamp}.log")
                else:
                    log_file = Path(f"logs/eon_{timestamp}.log")
...
def get_logger(name: str = "eon") -> logging.Logger:
    """
